...
public interface IDbService
{
    ...
    DataTable GetTableParam(string sql, IDictionary<string, object> parameters);
...
{
    public DataTable GetTableParam(string sql, IDictionary<string, object> parameters)
    {
        CheckConnectionString();

        DataTable datatable = new();
        using SqlConnection connection = new(DB_CONNECTION);    // ensures disposal
        using SqlCommand command = new(sql, connection); // Create a command
        ...
        using SqlDataAdapter adapter = new(command); // Use the command in the adapter
    ...
    public DataTable GetTable(string sql, params object?[] list)
    {
        ...
        DataTable datatable = new();
        using SqlConnection connection = new(DB_CONNECTION);    // ensures disposal
        using SqlCommand command = new(DB_SQL, connection);
        for (int i = 0; i < list.Length; i++)
        {
            command.Parameters.AddWithValue("@param" + i, list[i] ?? DBNull.Value);
        }
